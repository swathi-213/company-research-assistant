...
from product_research.open_deep_research.configuration import Configuration, SearchAPI
from product_research.open_deep_research.prompts import summarize_webpage_prompt
from product_research.open_deep_research.state import ResearchComplete, Summary
...
async def tavily_search(
    ...
    # Simplified: Use user API key directly instead of complex lookup
    model_api_key = configurable.user_api_key
...
DUCKDUCKGO_SEARCH_DESCRIPTION = (
    ...
    "Useful for when you need to answer questions about current events, companies, and general information. "
...
async def duckduckgo_search(
    ...
    """Fetch and summarize search results from DuckDuckGo search (free, no API key required).

    Args:
        queries: List of search queries to execute
        max_results: Maximum number of results to return per query
        topic: Topic filter for search results (general, news, or finance)
        config: Runtime configuration (not used for DuckDuckGo, but kept for consistency)
    ...
    for query in queries:
        try:
            # Use text search for general queries, news search for news topic
            if topic == "news":
                results = search_wrapper._ddgs_news(query, max_results=max_results)
            else:
                results = search_wrapper._ddgs_text(query, max_results=max_results)
            ...
            logging.warning(f"DuckDuckGo search error for query '{query}': {str(e)}")
            search_results.append({'query': query, 'results': []})
    
    # Step 2: Set up the summarization model with configuration
    configurable = Configuration.from_runnable_config(config)
    
    # Step 3: Format the final output (DuckDuckGo results are already concise)
...
def wrap_mcp_authenticate_tool(tool: StructuredTool) -> StructuredTool:
    ...
    async def authentication_wrapper(**kwargs):
        ...
        def _find_mcp_error_in_exception_chain(exc: BaseException):
...
async def load_mcp_tools(
    ...
    if not MCP_AVAILABLE or not MultiServerMCPClient:
...
def is_token_limit_exceeded(exception: Exception, model_name: str = None) -> bool:
    ...
    if model_name:
        ...
        if model_str.startswith('groq:') or 'llama-3' in model_str or 'mixtral' in model_str:
            provider = 'groq'
...
MODEL_TOKEN_LIMITS = {
    # Groq (Free Tier) - Primary models (Updated 2025)
    "llama-3.3-70b-versatile": 131072,  # Groq Llama 3.3 70B (current)
    "llama-3.3-8b-instant": 131072,  # Groq Llama 3.3 8B (current)
    "llama-3.1-70b-versatile": 131072,  # Deprecated - kept for compatibility
    "llama-3.1-8b-instant": 131072,  # Deprecated - kept for compatibility
    "mixtral-8x7b-32768": 32768,  # Groq Mixtral
    "groq:llama-3.3-70b-versatile": 131072,
    "groq:llama-3.3-8b-instant": 131072,
    "groq:llama-3.1-70b-versatile": 131072,  # Deprecated
    "groq:llama-3.1-8b-instant": 131072,  # Deprecated
    "groq:mixtral-8x7b-32768": 32768,
    # OpenAI (Legacy - kept for compatibility)
    "openai:gpt-5": 200000,  # adjust to your org's quota if different
    "openai:gpt-4o": 128000,
    "openai:gpt-4": 8192,
    # Anthropic Claude 4 Sonnet (Legacy)
    "anthropic:claude-sonnet-4-20250514": 200000,
    "anthropic:claude-3-7-sonnet": 200000,
    "anthropic:claude-3-5-sonnet": 200000,
    # Moonshot / Kimi K2 (Legacy)
